...
exports.validateStarknetConfig = validateStarknetConfig;
 ...
 */
function validateStarknetConfig(config) {
    const errors = [];
    if (!config) {
        errors.push({
            path: "starknet",
    ...
    if (config.network !== undefined) {
        ...
        if (!resolve_1.NETWORK_ALIASES[net] &&
            !net.startsWith("http://") &&
            !net.startsWith("https://")) {
            errors.push({
                path: "starknet.network",
                message: `Unknown network "${net}". Use one of: ${Object.keys(resolve_1.NETWORK_ALIASES).join(", ")}, or a full RPC URL (e.g., "http://127.0.0.1:5050").`,
            });
        }
    }
    ...
    if (config.wallet !== undefined) {
        ...
        if (typeof wallet !== "object" || wallet === null || Array.isArray(wallet)) {
            ...
            if (wallet.accountAddress !== undefined &&
