 ...
 */
task("compile:starknet", "Compile Cairo contracts with Scarb").setAction(async (_, hre) => {
  ...
  if (result.success) {
    ...
    throw new HardhatPluginError(PLUGIN_NAME, `Compilation failed:\n${result.output}`);
  }
});
 ...
 */
task("starknet:build", "Build Cairo contracts with Scarb (alias for compile:starknet)").setAction(
  async (_, hre) => {
