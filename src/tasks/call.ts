 ...
 */
task("starknet:call", "Call a read-only function on a Starknet contract")
  ...
  .setAction(async ({ contract: contractAddress, function: fnName, calldata }, hre) => {
    const args = calldata ? calldata.split(",").map((s: string) => s.trim()) : [];
 ...
 */
task("starknet:invoke", "Execute a state-changing function on a Starknet contract")
  .addParam("contract", "The contract address", undefined, types.string)
